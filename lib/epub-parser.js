		...
		*/
		...
		function parseContainer(err, containerJSON, finalCallback) {
			console.log('Parsing Container');
		      ...
		      parser.parseString(opfDataXML.toString(), function (err, opfJSON) {
				  ...
				  /* This whole section needed to be reordered since epPub 3 CAN have an NCX for downward compatibility, so the presence of an ncx does NOT
				  	automatically mean that it is an epub 2 document. Thus the order is now rearranged !! Address TOC CSS for ePub3 here !! */
			  	...
			  	if (isEpub3) {
			  		...
			  		/* ATTENTION CHECK for OPSRoot before relevant entries! E.g. CSS @ Moby Dick */

			  		var temptest = "";
			  		if (epub3NavHtml.match(/href="css/)) {
			  			console.log("TREFFER");
			  			temptest = epub3NavHtml.replace(/href="css/g, "href=\""+opsRoot+"css");
			  		}
			  		if (temptest) {epub3NavHtml = temptest;}
		...
		function buildItemHashes() {
			for(item in itemlist) {
				...
				// A first try at fixing images, unfortunately they are in the html itself
				/*
				if (typeof mediaType !== 'undefined') {
					if (mediaType.substr(0,5) == 'image') {
				...
				*/
				...
				if(typeof properties !== 'undefined') {
					if(properties == 'cover-image') {
						...
						epub3NavId = id;
						epub3NavHtml = extractText(opsRoot+href); // This is where the epub3 navigation is copied, maybe inject OPSRoot + Cache here...?
						navPath = opsRoot + href.substr(0, href.indexOf('\/')+1); //Store path of the navigation doc, since paths in it are relative to here
					}
				}
